...
pp_train <- training2_pre$pheno1 %>% as.matrix()
...
def compute_PIs(X,Y,X1,alpha = 0.1, K=5, fit_muh_fun = leastsq_minL2):
    n = len(Y)
    #n1 = X1.shape[0]
    n1 = len(X1)
    ###############################
    ...
    for i in range(K):
        ...
        X1_2d = X1[:, np.newaxis]
        muh_vals_LKO = fit_muh_fun(X0_2d, Y0, 
                                   np.r_[X0d_2d, X1_2d])
