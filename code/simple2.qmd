    ...
    for (trait_num in 1:n_traits){
        ...
        for (i in 1:nrow(testing2)) {
            ...
            test_list[[i]] <- preds_all %>%
                dplyr::ungroup() %>% # ungroup by fold so that we get only one row at the end!
...
library(ggplot2)
tt_intermediate %>%
    ggplot(aes(x = training_set_size, y = interval_width, color = trait_num)) + geom_point() + geom_boxplot() + theme_bw()
```
