...
We'll simulate a x vector, then simulate y|x for 10,100 subjects.
We then hold out the 100 test set subjects, which are randomly chosen from the
10,100.

The remaining 10,000 constitute the training set. We do CV+ (5-fold) with them, then
construct prediction intervals for the predicted values for the test set subjects.

We also do an analysis with exactly 100 training set subjects. That is, we 
do CV+, 5-fold, with the 100 subjects before constructing intervals for the 
test set.
...
We now sample `r n_test` test subjects.
...
predicted values per test set subject. We consider the test set 
subjects one at a time, and for each one, we get the predicted values.
